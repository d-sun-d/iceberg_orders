...
class Order(object):
    def __init__(self, buy_sell, order_id, price, size, hidden_size=None):
    ...
    def order_from_row(row):
        ...
        if len(parts) == 4:
            ...
            return Order(buy_sell, order_id, price, size)
        else:
            buy_sell, order_id, price, hidden_size, size  = parts
            return  Order(buy_sell, order_id, price, size, hidden_size)
...
class ListBasedOrderQueue(object):
    ...
    def _cmp(self, order1, order2):
        ...
        return order if self.buy_sell == "B" else (not order)
...
class Stock(object):
    ...
    def process_row(self, row):
        order = Order.order_from_row(row)
